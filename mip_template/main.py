...
def solve(dat):
    ...
    if params["Sample Two Values Parameter"] == "Value 1":
        ...
        sample_output_table_df = sample_input_table_df[
            ["Primary Key Two", "Data Field Two"]
        ]
    else:
        raise ParameterError(
            f"bad value for 'Sample Two Values Parameter': {params['Sample Two Values Parameter']}"
        )
    sample_output_table_df.rename(
        columns={"Primary Key One": "Primary Key", "Data Field One": "Data Field"},
        inplace=True,
    )
